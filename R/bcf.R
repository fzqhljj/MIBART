sourceCpp("C:/Users/jj-li/Desktop/bcf_1.3.1/bcf/src/bcf_clean_overpar.cpp")
.ident <- function(...){
...
bcf <- function(y, z, x_control, x_moderate=x_control, pihat,
  ...
  fitbcf = bcfoverparRcppClean(yscale[perm], z[perm], t(x_c[perm,]), t(x_m[perm,,drop=FALSE]), t(x_m[1,,drop=FALSE]),
                        ...
                        mod_sd = ifelse(abs(sdy - sd_moderate)<1e-6, 1, sd_moderate/sdy), # if HN make sd_moderate the prior median
                        base_control, power_control, base_moderate, power_moderate, 
                        tempdir(), status_interval = update_interval)
  print("bcfClean finished")
  #B = drop(fit$post_B)
  #B0 = fit$b0
  #EYs = fit$post_yhat
