...
List bcfoverparRcppClean(NumericVector y_, NumericVector z_,
                  ...
                  bool RJ= false, bool prior_sample=false,
                  double trt_init = 1.0)
{   
  std::string treef_name = as<std::string>(treef_name_);
  std::ofstream treef(treef_name.c_str());
  Rcout<< "MCMC begin"<<endl;
/*  bool randeff = true;
  if(random_var_ix.n_elem == 1) {
    randeff = false;
    Rcout<< "No randeff"<<endl;//In our project we don't use randeff.
  }
  ...
  *****************************************************************************/
  ...
  for(NumericVector::iterator it=y_.begin(); it!=y_.end(); ++it) {
    y.push_back(*it);
    if(*it<miny) miny=*it;//update min
    if(*it>maxy) maxy=*it;//update max
  ...
  *****************************************************************************/
  ...
  Rcout << xi_con[0].size()<< xi_con[1].size()<< xi_con[2].size()<< xi_con[3].size()<<"test xi_con"<<endl;
  /*****************************************************************************
  /* Read, format X_mod
  *****************************************************************************/
  int ntrt = 0;
...
//      random_var_post.row(save_ctr) = (sqrt( eta % eta % random_var)).t();
  ...
  treef.close();
  Rcout<<"MCMC over"<<endl;
  return(List::create(_["yhat_post"] = yhat_post,_["m_post"] = m_post, _["b_post"] = b_post, _["b_est_post"] = b_est_post,
